...
export default function() {
  ...
  // Generate non-overlapping contours using layered approach with value mapping
  function generateNonOverlappingContours(values, thresholds) {
    ...
    // Apply value mapping to thresholds before processing
    const mappedValues = mapValuesToThresholds(extendedValues, thresholds);
    ...
    for (let i = 0; i < extendedDx * extendedDy; i++) {
      ...
      if (mappedValue == null || !isFinite(mappedValue)) {
        levelMap[i] = -1; // Mark as invalid
        continue;
      }
      
      // Find which threshold level this mapped value belongs to
      let level = -1;
      for (let j = 0; j < thresholds.length; j++) {
        if (mappedValue >= thresholds[j]) {
          level = j;
        } else {
          break;
        }
    ...
    // Generate contours for each level boundary using mapped values
    for (let level = 0; level < thresholds.length; level++) {
      const contour = generateLevelContour(mappedValues, levelMap, thresholds[level], level, extendedDx, extendedDy);
  ...
  // Map original values to threshold values according to mapping rules
  function mapValuesToThresholds(values, thresholds) {
    if (!thresholds || thresholds.length === 0) {
      return values; // No mapping if no thresholds
    }
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      // 3. If value is in a range [threshold[i], threshold[i+1]), map to threshold[i+1]
        ...
        mappedValues[i] = mappedThreshold;
      }
    }
