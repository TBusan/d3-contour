...
export default function enhancedContours() {
    ...
    function isorings(values, value, callback) {
        ...
        while (++y < dy - 1) {
            x = -1;
            while (++x < dx - 1) {
                if (nullMask && !isValidCell(x, y)) continue;
    ...
    function getValue(values, x, y) {
        if (values == null || x < 0 || x >= dx || y < 0 || y >= dy) return NaN;
        const value = values[y * dx + x];
        return nullMask && nullMask[y * dx + x] === 0 ? NaN : value;
    }
    
    function isValidCell(x, y) {
        return getValue(null, x, y) !== null &&
               getValue(null, x + 1, y) !== null &&
               getValue(null, x, y + 1) !== null &&
               getValue(null, x + 1, y + 1) !== null;
    }
