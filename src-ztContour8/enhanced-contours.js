...
export default function() {
  ...
  // Generate non-overlapping contours using traditional method + post-processing
  function generateNonOverlappingContours(values, thresholds) {
    ...
    // Temporarily update grid dimensions
    const originalDx = dx, originalDy = dy;
    dx = extendedDx; dy = extendedDy;
    
    // Generate traditional contours for each threshold
    const rawContours = [];
    for (let i = 0; i < thresholds.length; i++) {
      const threshold = thresholds[i];
      const result = contour(extendedValues, threshold);
      
      if (result.coordinates.length > 0) {
        result.properties = { value: threshold, level: i };
        rawContours.push(result);
      }
    }
    ...
    // Post-process to remove overlaps by clipping higher-level contours
    const processedContours = removeContourOverlaps(rawContours, extendedValues);
    
    // Adjust coordinates if boundaries were extended
    if (extendBoundaries) {
      processedContours.forEach(contour => {
        contour.coordinates = adjustExtendedCoordinates(contour.coordinates);
      });
    }
  ...
  // Generate contour using traditional threshold method
  function generateThresholdContour(values, threshold, gridDx, gridDy) {
    var polygons = [],
        holes = [];
    ...
    // Use traditional isorings method
    isorings(values, threshold, function(ring) {
  ...
  // Enhanced contour generation with improved algorithms
  ...
  function isorings(values, value, callback) {
    ...
    function processCase(caseIndex, localX, localY, cellValues, stitch) {
      // Ensure caseIndex is within valid range
  ...
  // Helper functions for non-overlapping contours
  function isoringsForLevel(values, levelMap, threshold, targetLevel, callback) {
    var fragmentByStart = new Map(),
        fragmentByEnd = new Map(),
        x, y, t0, t1, t2, t3;

    // Use proper grid dimensions for level-based contour generation
    const gridDx = dx;
    const gridDy = dy;
    ...
    while (++y < gridDy - 1) {
      x = -1;
      t1 = isLevelAbove(levelMap[y * gridDx + gridDx], targetLevel);
      t2 = isLevelAbove(levelMap[y * gridDx], targetLevel);
      processCase(t1 << 1 | t2 << 2, x, y, [], stitch);
      while (++x < gridDx - 1) {
        t0 = t1, t1 = isLevelAbove(levelMap[y * gridDx + gridDx + x + 1], targetLevel);
        t3 = t2, t2 = isLevelAbove(levelMap[y * gridDx + x + 1], targetLevel);
        
        const caseIndex = t0 | t1 << 1 | t2 << 2 | t3 << 3;
        const cellValues = [
          values[y * gridDx + x],
          values[y * gridDx + x + 1],
          values[y * gridDx + gridDx + x],
          values[y * gridDx + gridDx + x + 1]
        ];
    ...
    // Special case for the last row (y = gridDy - 1, t0 = t1 = 0).
    x = -1;
    t2 = isLevelAbove(levelMap[y * gridDx], targetLevel);
    processCase(t2 << 2, x, y, [], stitch);
    while (++x < gridDx - 1) {
    ...
    function processCase(caseIndex, localX, localY, cellValues, stitch) {
      ...
      if (enableSaddleDisambiguation && (caseIndex === 5 || caseIndex === 10) && cellValues.length === 4) {
        ...
        if (saddleCase && Array.isArray(saddleCase)) {
          saddleCase.forEach(stitch);
        }
      } else {
        const caseLines = cases[caseIndex];
  ...
  // Remove overlaps between contours by geometry clipping
  function removeContourOverlaps(contours, values) {
    if (contours.length <= 1) return contours;
    
    // Sort contours by threshold value (ascending)
    contours.sort((a, b) => a.value - b.value);
    
    // For now, return all contours (traditional behavior)
    // In future, implement geometric clipping to remove overlaps
    return contours;
  }
